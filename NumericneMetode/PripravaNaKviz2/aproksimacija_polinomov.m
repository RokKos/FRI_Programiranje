function aproksimacija_polinomov
...
%
% =============================================================================
% Aproksimacija polinomov
% =====================================================================@019279=
% 1. podnaloga
% Naj bosta `x` in `y` vektorja podatkov in funkcijskih izracunov,
% cez katere zelimo napeti polinom stopnje `d`.
% Napisi funkcijo `p = poly_lsq(x, y, d)`, ki vrne `d+1` koeficientov polinoma
% `p`, ki se po metodi najmanjsih kvadratov najbolje prilega danim
% podatkom.
% 
...
for stopnja = 1:20
    p = poly_lsq(x, y, stopnja);
    polinom = polyval(p,x)
    plot(x, polinom, 'Color', [1.0/stopnja 0 1.0/stopnja])
end
...
% =====================================================================@019280=
% 2. podnaloga
% Primerjaj obcutljivost sistema za visoke stopnje polinomov.
% Aproximiraj funkcijo $\sin$ na $[-2\pi, 2\pi]$ v 40 tockah s polinomi 
% cedalje visjih stopenj. Uporabi normalni sistem in razcep Choleskega,
% QR, SVD in matlabov `\` za resevanje predolocenega sistema.
% Narisi napako v odvisnosti od $d$, na locen graf pa se obcutljivost
% sistema.
% =============================================================================



    function p = normalni_sistem(x,y, d)
        x_len = length(x);
        A = ones(x_len, d);
    ...
    function p = singular_value_decomposition(x,y,d)
        x_len = length(x);
        A = ones(x_len, d);
...
p_prev = poly_lsq(x, y, 7)
normalni_p = normalni_sistem(x,y, 7)
QR_p = QR_razcep(x,y, 7)
SVD_p = singular_value_decomposition(x,y, 7)
...
function validation = validate_current_attempt_file(src)
  ...
  check_initialize(file_parts);
      ...
      if check_part()
        check.current_part.token = 'eyJ1c2VyIjoxMDIyLCJwYXJ0IjoxOTI3OX0:1gfLt1:uVBAyIzo2xuZOzCWDFOMRAZqjB4';
        try
              check_equal('poly_lsq(linspace(0,4*pi,10)'', 5*linspace(0,4*pi,10)''+6, 1)', [5; 6]);
  % Y__ = 0.4*X__.^2 + 0.3*X__ - 0.6;
  % check_equal('poly_lsq(X__, Y__, 1)', polyfit(X__, Y__, 1));
  ...
  % check_secret(multiply(500, 123));
      ...
      if check_part()
        check.current_part.token = 'eyJ1c2VyIjoxMDIyLCJwYXJ0IjoxOTI4MH0:1gfLt1:J3nnIDuNC6wL5XtAXYoWX5PdawA';
        try
