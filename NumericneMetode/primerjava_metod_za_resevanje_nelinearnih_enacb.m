function primerjava_metod_za_resevanje_nelinearnih_enacb
...
%
% =============================================================================
% Primerjava metod za resevanje nelinearnih enacb
%
% Pripravi  implementacija naslednjih iteracijskih metod:
% <ul>
%   <li>tangentna metoda: $x_{r+1} = x_r - f(x_r)/f'(x_r)$</li>
%   <li>sekantna metoda: $x_{r+1} = x_r - f(x_r)(x_r - x_{r-1})/(f(x_r) - f(x_{r-1}))$</li>
%   <li>metoda $(f, f', f'')$: $x_{r+1} = x_r - f(x_r)/f'(x_r) - f''(x_r)f(x_r)^2/(2f'(x_r)^3)$</li>
%   <li>Halleyjeva metoda: $x_{r+1} = x_r - 2f(x_r)f'(x_r)/(2f'(x_r)^2 - f(x_r)f''(x_r))$</li>
% </ul>
% =====================================================================@018563=
% 1. podnaloga
% Z vsemi metodami poiscite priblizek za niclo funkcije $f(x) =
% x+4-exp(x^2)$ pri zacetnem priblizku $x_0 = 1$ (in $x_1 = 1.1$ za
% sekantno metodo), ki se cimbolje ujema s priblizkom za niclo funkcije
% $f$, ki ga dobite z uporabo `fzero`. Primerjajte stevilo korakov, ki jih
% metode potrebujejo za dosego primerljivih priblizkov.
% 
% Pripravi skripto, v kateri za vsako metodo narisete graf stevila korakov,
% ki zagotavljajo dober priblizek, v odvisnosti od zacetnega priblizka. Za
% zacetne priblizke izberite vrednosti med 1 in 10 v ekvidistantnih
% razmikih dolzine 0.1. Na podlagi grafa ocenite ucinkovitost metod.
% 
% Skripto dopolnite tako, da izrise tudi grafa stevila funkcijskih
% izracunov in dejanskega casa izvajanja iteracije v odvisnosti od zacetnih
% priblizkov. Kako bi na podlagi teh grafov ocenil ucinkovitost metod?
% 
% Pripravljen test metod uporabite se za iskanje nicle kake druge funkcije 
% $f$, na primer funkcije, podane s predpisom $f(x) = e^x + 2^{-x} + 2\cos(x)
% - 6$.
% =============================================================================
...
f = @(x) x + 4 - exp(x^2);
f_odv = @(x) 1 - 2* x * exp(x^2);
f_odv_2 = @(x) -4* x^2 * exp(x^2) - 2 * exp(x^2);
x_0 = 1;
x_1 = 1.1;
epsilon = 1e-5;
max_iteration = 200;
...
nicla = fzero(f, x_0);
    ...
    function [nic, koraki, sez_priblizki] = tangentna_metoda(g, g_odv, x_0, epsilon, max_iteration, nicla)
        sez_priblizki = zeros(max_iteration, 1);
        sez_priblizki(1) = x_0;
        for i = 2:max_iteration
            x_r = sez_priblizki(i - 1);
            sez_priblizki(i) = x_r - g(x_r) / g_odv(x_r);
            %tako bi sli ce nebi vedeli resitve if (abs(sez_priblizki(i) - sez_priblizki(i-1)) < epsilon * sez_priblizki(i-1))
            if (abs(sez_priblizki(i) - nicla) < epsilon)
                sez_priblizki = sez_priblizki(1:i);
                nic = sez_priblizki(end);
                koraki = i;
        ...
        nic = sez_priblizki(end);
        koraki = max_iteration;
        disp('Dosegli smo max iteration.')
        
    end

[tang_nicla, tang_koraki, tang_priblizki] = tangentna_metoda(f, f_odv, x_0, epsilon, max_iteration, nicla);

    function [nic, koraki, sez_priblizki] = sekantna_metoda(g,x_0, x_1, epsilon, max_iteration, nicla)
        sez_priblizki = zeros(max_iteration, 1);
        sez_priblizki(1) = x_0;
        sez_priblizki(2) = x_1;
        for i = 3:max_iteration
            x_r = sez_priblizki(i - 1);
            x_r_1 = sez_priblizki(i - 2);
            sez_priblizki(i) = x_r - g(x_r)*(x_r - x_r_1) /(g(x_r) - g(x_r_1));
...
[sekantna_nicla, sekantna_koraki, sekantna_priblizki] = sekantna_metoda(f, x_0, x_1, epsilon, max_iteration, nicla);


    function [nic, koraki, sez_priblizki] = metoda_treh_odvodov(g, g_odv, g_odv_2,x_0, epsilon, max_iteration, nicla)
        sez_priblizki = zeros(max_iteration, 1);
        sez_priblizki(1) = x_0;
        for i = 2:max_iteration
            x_r = sez_priblizki(i - 1);
            sez_priblizki(i) = x_r - g(x_r)/g_odv(x_r) - g_odv_2(x_r) * g(x_r)^2 / (2 * g_odv(x_r)^3);
            %if (abs(sez_priblizki(i) - sez_priblizki(i-1)) < epsilon * sez_priblizki(i-1))
            if (abs(sez_priblizki(i) - nicla) < epsilon)
                sez_priblizki = sez_priblizki(1:i);
                nic = sez_priblizki(end);
...
[tri_odvodi_nicla, tri_odvodi_koraki, tri_odvodi_priblizki] = metoda_treh_odvodov(f, f_odv, f_odv_2, x_0, epsilon, max_iteration, nicla);


    function [nic, koraki, sez_priblizki] = halleyeva_metoda(g, g_odv, g_odv_2,x_0, epsilon, max_iteration, nicla)
        sez_priblizki = zeros(max_iteration, 1);
        sez_priblizki(1) = x_0;
        for i = 2:max_iteration
            x_r = sez_priblizki(i - 1);
            sez_priblizki(i) = x_r - 2 * g(x_r) * g_odv(x_r) / (2 * g_odv(x_r)^2 - g(x_r)* g_odv_2(x_r));
            %if (abs(sez_priblizki(i) - sez_priblizki(i-1)) < epsilon * sez_priblizki(i-1))
            if (abs(sez_priblizki(i) - nicla) < epsilon)
                sez_priblizki = sez_priblizki(1:i);
                nic = sez_priblizki(end);
...
[halleyeva_nicla, halleyeva_koraki, halleyeva_priblizki]= halleyeva_metoda(f, f_odv, f_odv_2, x_0, epsilon, max_iteration, nicla);
...
%plot (abs(tang_priblizki - nicla), 'sk');
%plot (abs(sekantna_priblizki - nicla), 'or');
%plot (abs(tri_odvodi_priblizki - nicla), 'xg');
%plot (abs(halleyeva_priblizki - nicla), 'vb');

x_os = 1.0:0.1:10.0;
tan = 1, sek = 2, tri = 3, hal = 4;
its = zeros(size(x_os, 4));
times = zeros(size(x_os, 4));

for j = 1:length(x_os); 
    x_s = x_os(j);
    tic
    [tan_nicla, tan_koraki, tan_priblizki] = tangentna_metoda(f, f_odv, x_s, epsilon, max_iteration, nicla);
    times(j, tan) = toc;
    its(j, tan)=tan_koraki;
    
    
    tic
    [sekantna_nicla, sekantna_koraki, sekantna_priblizki] = sekantna_metoda(f, x_s, 1.1*x_s, epsilon, max_iteration, nicla);
    times(j, sek) = toc;
    its(j, sek)=sekantna_koraki;
    
    
    tic
    [tri_odvodi_nicla, tri_odvodi_koraki, tri_odvodi_priblizki] = metoda_treh_odvodov(f, f_odv, f_odv_2, x_s, epsilon, max_iteration, nicla);
    times(j, tri) = toc;
    its(j, tri)=tri_odvodi_koraki;
    
    
    tic
    [halleyeva_nicla, halleyeva_koraki, halleyeva_priblizki]= halleyeva_metoda(f, f_odv, f_odv_2, x_s, epsilon, max_iteration, nicla);
    times(j, hal) = toc;
    its(j, hal)=halleyeva_koraki;
    
end
    
plot (x_os, its(:,tan), 'sk');
plot (x_os, its(:,sek), 'or');
plot (x_os, its(:,tri), 'xg');
plot (x_os, its(:,hal), 'vb');
xlabel('iter');
ylabel('napaka');
...
plot (x_os, times(:,tan), 'sk');
plot (x_os, times(:,sek), 'or');
plot (x_os, times(:,tri), 'xg');
plot (x_os, times(:,hal), 'vb');
...
function validation = validate_current_attempt_file(src)
  ...
  check_initialize(file_parts);
      ...
      if check_part()
        check.current_part.token = 'eyJ1c2VyIjoxMDIyLCJwYXJ0IjoxODU2M30:1gL34h:Zi80PVS6eNmpGBkZnosnSJCEqoM';
        try
              % % =====================================================================@000000=
