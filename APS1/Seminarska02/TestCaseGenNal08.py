...
def pick_element(pool_of_possibilities):
	is_not = (OPERATOR_NOT in pool_of_possibilities) and (randint(0,100) < 75)
	if (is_not): return OPERATOR_NOT
	else:
		el_index = randint(0, len(pool_of_possibilities) - 1)
		return pool_of_possibilities[el_index]
...
if __name__ == "__main__":
	...
	while (var_num > 0 and op_num > 0):
		...
		element = pick_element(pool_of_possibilities)
