...
% =====================================================================@018949=
% 1. podnaloga
% Oglejte si obcutljivost Hilbertovih matrik, ki jih dobite z ukazom
% `hilb(n)` na narascajoce `n`. Narisi graf! Za izbran `n` in 
% vektor `x` izracunajte $y = H_n x$ in z operatorjem \
% poiscite resitev $x' = H_n^{-1} y$. Koliko se $x$ in $x'$ razlikujeta po
% normi?
% =============================================================================

% =====================================================================@018951=
% 2. podnaloga
% Vandermondova matrika za vektor $v$ je enaka $[v_i^{n-j}]$. Oglejte si
% obcutljivost Vandermondovih matrik za razlicne vektorje $v$.
% S pomocjo ukazov `vander` in `cond` primerjajte
% obcutljivosti matrik za vektorja `1:n` in $e^{2\pi ki/n}$, za $k$ od $1$
% do $n$. Preverite da je ena od teh matrik do skaliranja unitarna!
% =============================================================================
...
function validation = validate_current_attempt_file(src)
  ...
  check_initialize(file_parts);
      ...
      if check_part()
        check.current_part.token = 'eyJ1c2VyIjoxMDIyLCJwYXJ0IjoxODk0OX0:1gNbJM:ylaq4Z6iS378yaeDEIXRg5m_o6Q';
        try
  ...
  check_secret(zmnozi(500, 123));
      ...
      if check_part()
        check.current_part.token = 'eyJ1c2VyIjoxMDIyLCJwYXJ0IjoxODk1MX0:1gNbJM:XHfRsLqvV3AJz0wemXDDQ92BISg';
        try
