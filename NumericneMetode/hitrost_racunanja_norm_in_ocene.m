function hitrost_racunanja_norm_in_ocene
...
%
% =============================================================================
% Hitrost racunanja norm in ocene
% =====================================================================@018948=
% 1. podnaloga
% Za izbrani vektor $x$ izracunajte $\|x\|_1$, $\|x\|_2$, $\|x\|_\infty$.
% Izracunajte $\|A\|_1$, $\|A\|_\infty$, $\|A\|_F$ in $N_\infty(A)$
% za $A = [2 1 3; 5 4 1; -2 -1 2]$ in ocenite $\|A\|_2$. Koliko se ocena
% razlikuje od dejanske vrednosti $\|A\|_2$, ce jo izracunate z ukazom
% `norm`?
% 
% Primerjajte cas razunanja norm za nakljucno 1000x1000 matriko.
% =============================================================================
...
r3 = norm_inf(x)
a1 = mat_norm_1(A)
a2 = mat_norm_frobenius(A)
a3 = mat_norm_inf(A)
a4 = n_inf_mat(A)

%ocene
[n,m] = size(A);
fn = {@mat_norm_frobenius, @mat_norm_inf, @mat_norm_1, @n_inf_mat};
coeff = [1/sqrt(n), 1;
         1/sqrt(n), sqrt(n);
         1/sqrt(n), sqrt(n);
         1, n
         ]
ime_funkcije = {'Po frobeniusu:', '2:', '3:', '4:'};

for i = (1:4)
    norma = fn{i}(A);
    spodnja_meja = norma * coeff(i,1);
    zgornja_meja = norma * coeff(i,2);
    diplay = [ime_funkcije(i),'A_norm_2 med', spodnja_meja, 'in', zgornja_meja];
    disp(diplay);
...
function validation = validate_current_attempt_file(src)
  ...
  check_initialize(file_parts);
      ...
      if check_part()
        check.current_part.token = 'eyJ1c2VyIjoxMDIyLCJwYXJ0IjoxODk0OH0:1gNaH3:HvRBwjeLCVncyGxq1GSB2shdZpc';
        try
